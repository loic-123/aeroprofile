...
def solve_chung_ve(
    ...
    if prior_active:
        try:
            ...
            logger.info("  pass0 MLE: CdA=%.3f σ=%.3f Crr=%.5f R²=%.3f",
                        raw_cda, _rsig, raw.crr, raw.r_squared_elev)
    ...
    logger.info("  pass1 base: CdA=%.3f σ=%.3f Crr=%.5f R²=%.3f",
                result.cda, _p1sig, result.crr, result.r_squared_elev)
    adaptive_factor = 1.0
            ...
            if ratio <= 1.0:
                ...
                try:
                    ...
                    logger.info("  pass2 done: CdA=%.3f σ=%.3f Crr=%.5f R²=%.3f",
                                result2.cda, _p2sig, result2.crr, result2.r_squared_elev)
                    result = result2
